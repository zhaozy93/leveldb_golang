...
func NewTableCache(dbName string) *TableCache {
	...
	tableCache.cache, _ = lru.New(internal_key.MaxOpenFiles - internal_key.NumNonTableCacheFiles)
	return &tableCache
}
...
func (tableCache *TableCache) findTable(fileNum uint64) (*sstable.SsTable, error) {
	...
	if ok {
		...
		ssTable, err := sstable.SsTabelOpen(internal_key.TableFileName(tableCache.dbName, fileNum))
