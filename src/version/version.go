...
type FileMetaData struct {
	...
	number     uint64
	fileSize   uint64
	smallest   *internal_key.InternalKey
	largest    *internal_key.InternalKey
}
...
type Version struct {
	...
	seq            uint64
	files          [internal_key.NumLevels][]*FileMetaData
	// Per-level key at which the next compaction at that level should start.
	// Either an empty string, or a valid InternalKey.
	compactPointer [internal_key.NumLevels]*internal_key.InternalKey
}
...
func LoadVersion(dbName string, number uint64) (*Version, error) {
	fileName := internal_key.DescriptorFileName(dbName, number)
...
func (v *Version) Save() (uint64, error) {
	...
	fileName := internal_key.DescriptorFileName(v.tableCache.dbName, v.nextFileNumber)
	v.nextFileNumber++
...
func (v *Version) NextSeq() uint64 {
	return atomic.AddUint64(&v.seq, 1)
}
	...
	for level := 0; level < internal_key.NumLevels; level++ {
		...
		if level == 0 {
			...
			for i := 0; i < numFiles; i++ {
				f := v.files[level][i]
				if internal_key.UserKeyComparator(key, f.smallest.UserKey) >= 0 && internal_key.UserKeyComparator(key, f.largest.UserKey) <= 0 {
					tmp = append(tmp, f)
				}
			}
			if len(tmp) == 0 {
			...
			if index >= numFiles {
				...
				if internal_key.UserKeyComparator(key, tmp2[0].smallest.UserKey) < 0 {
					files = nil
					numFiles = 0
				} else {
					files = tmp2[:]
...
func (v *Version) findFile(files []*FileMetaData, key []byte) int {
	...
	for left < right {
		mid := (left + right) / 2
		f := files[mid]
		if internal_key.UserKeyComparator(f.largest.UserKey, key) < 0 {
